...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{/guards}: Houses route-activation logic, preventing unauthenticated routing (\texttt{authGuard}) or block-pending routing restrictions (\texttt{verifiedGuard}).
...
\textbf{Dependency Injection.} Angular's DI container is perhaps the most pervasive pattern in the framework. Services, guards, interceptors, and resolvers are all instantiated and injected by the framework based on their provider configuration. In the NEST frontend, this allows the \texttt{authInterceptor} to seamlessly inject the \texttt{Router} to handle unauthenticated session redirects without hardcoding navigation dependencies.
...
\begin{enumerate}[label=\textbf{Step \arabic*:}, leftmargin=*]
    ...
    \item \textbf{Intercept Routing:} The frontend \texttt{verifiedGuard} intercepts all navigation attempts. Identifying that the user's \texttt{isVerified} flag is false, the guard redirects the user to the pending interface.
    ...
    \item \textbf{Activate Session:} The backend updates the User's \texttt{isVerified} flag to true and assigns the \texttt{blockId}. The \texttt{verifiedGuard} clears the routing restriction, granting the user full access to community modules.
\end{enumerate}
...
Theme switching is managed reactively via the \texttt{ThemeService} (\path{frontend/src/app/core/services/theme.service.ts}). It uses an Angular Signal to track the current theme state and an Angular \texttt{effect()} to write the active class attribute to the document element, persisting the user's choice in browser storage:
