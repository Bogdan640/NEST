...
\centering
\begin{tikzpicture}[
    usecase/.style={draw, ellipse, minimum width=2.5cm, minimum height=1cm, align=center, font=\scriptsize},
    actor/.style={draw=none, align=center, font=\scriptsize},
    >=Stealth, node distance=1.5cm and 3cm]
    
    % Actors
    \node[actor] (guest) {\textbf{Guest}};
    \node[actor, below=2cm of guest] (resident) {\textbf{Resident}};
    \node[actor, below=2cm of resident] (admin) {\textbf{Administrator}};
    
    % Use Cases
    \node[usecase, right=2cm of guest] (uc_register) {Register / Log In};
    \node[usecase, right=3cm of resident] (uc_join) {Request to Join Block};
    \node[usecase, above=0.3cm of uc_join] (uc_feed) {View/Post in Feed};
    \node[usecase, below=0.3cm of uc_join] (uc_shed) {Borrow/Lend Tools};
    \node[usecase, below=0.3cm of uc_shed] (uc_event) {RSVP to Events};
    \node[usecase, right=2cm of admin] (uc_approve) {Approve Join Requests};
    \node[usecase, below=0.3cm of uc_approve] (uc_manage) {Manage Building};
    ...
    \draw[->, dashed] (admin.north) -- node[left, font=\tiny] {Inherits} (resident.south);
