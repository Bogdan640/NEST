...
\textbf{Multi-Page Applications (MPAs)} represent the earliest model, where each user interaction triggers a full page reload from the server. While simple to reason about, this approach results in noticeable latency and a disjointed user experience, because the entire HTML document must be re-fetched for every navigation action.

\textbf{Single-Page Applications (SPAs)} emerged as a response. In this model, the browser loads a single HTML shell and then dynamically rewrites the page content using JavaScript. Navigation occurs client-side through the History API, and data is fetched asynchronously from the server through API calls \cite{angularDocs}. The perceived responsiveness is substantially better, since only data payloads travel over the network rather than complete rendered pages.
...
Three frameworks dominate this space: React (maintained by Meta), Vue.js, and Angular (maintained by Google). Table~\ref{tab:framework-comparison} summarizes their key differences.
...
\section{Design Patterns in Web Engineering}

Several classical design patterns from the ``Gang of Four'' catalog \cite{gamma1994design} find direct application in the architecture of modern web applications.

\textbf{Singleton.} Angular's dependency injection system guarantees that services declared with \texttt{providedIn: 'root'} are instantiated exactly once and shared across the entire application. This is critical for services such as the authentication facade or the theme service, where multiple instances would produce conflicting state.

\textbf{Observer.} The NgRx store is fundamentally an implementation of the Observer pattern. Components \textit{subscribe} to slices of the state, and the store \textit{notifies} them whenever the relevant state changes. In the reactive programming terminology used by Angular, this is expressed through RxJS Observables (or, equivalently, through signals).

\textbf{Strategy.} The Express middleware pipeline on the backend exemplifies the Strategy pattern. Each middleware function (authentication, verification, rate limiting) encapsulates a specific policy, and they are composed in different combinations depending on the route's security requirements.

\textbf{Dependency Injection.} Angular's DI container is perhaps the most pervasive pattern in the framework. Services, guards, interceptors, and resolvers are all instantiated and injected by the framework based on their provider configuration. This makes it straightforward to substitute implementations (e.g., a mock API service during testing) without modifying the consuming code \cite{fowler2002patterns}.
