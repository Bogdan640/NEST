...
\begin{itemize}
    \item \textbf{Node.js \& npm:} Version 20.x is utilized as the backend runtime, while npm serves as the package manager and scripts orchestrator.
    \item \textbf{TypeScript Compiler (tsc):} Strict compilation rules are enforced across both frontend and backend projects (e.g., \texttt{"strict": true}, \texttt{"noImplicitAny": true}), catching type-coercion bugs and null reference pointer exceptions before the code is executed.
    \item \textbf{Angular CLI:} Controls development compilation, serving, lazy-loaded chunk generation, and optimal Ahead-of-Time (AoT) production bundling.
    \item \textbf{Prisma Engine:} Instantiated as a development dependency, coordinating SQL schema synchronization, SQLite database seeding, and generating the strongly typed database client API.
    \item \textbf{SQLite Engine:} Utilized for relational persistence, running in-process to simplify local staging and deployment footprints while supporting complex transaction models.
\end{itemize}
...
To illustrate this architecture, Listing \ref{lst:page-header} shows the code implementation of the \texttt{PageHeaderComponent} (\texttt{frontend/src/app/shared/components/page-header/page-header.component.ts}), which uses standalone component architecture and modern Signal inputs.

\begin{lstlisting}[caption={Page Header Component featuring Angular Signal Inputs}, label={lst:page-header}]
...
By utilizing \texttt{input.required<string>()} and \texttt{input<string>('')}, properties are exposed as read-only Signals. Component templates read these values using function invocation syntax (e.g., \texttt{title()}), establishing a dependency link that automatically updates the specific template node when the underlying input changes.
...
To decouple components from store implementation details, the architecture applies the **Facade Pattern** \cite{gamma1994design}. Listing \ref{lst:auth-facade} displays the implementation of the \texttt{AuthFacade} (\texttt{frontend/src/app/store/auth/auth.facade.ts}), illustrating how it exposes read-only reactive properties as Signals using `selectSignal()`.
...
\begin{enumerate}
    \item \textbf{Registration:} The user registers an account, which is created with a \texttt{PENDING} status.
    \item \textbf{Join Block Guard:} Upon logging in, Angular's \texttt{PendingGuard} intercepts routing. If the user's \texttt{blockId} is null, they are redirected to the block selection interface.
    \item \textbf{Join Request Validation:} The user enters their physical block's unique alphanumeric code. The system creates a \texttt{JoinRequest} entity and displays a pending screen.
    \item \textbf{Admin Authorization:} A block administrator reviews the pending requests in their Admin Panel, approving or rejecting the registration based on residency records.
    \item \textbf{Session Activation:} Upon approval, the user's status transitions to \texttt{isVerified = true}, and route guards automatically grant access to the community modules.
\end{enumerate}
...
\begin{picture}(14,2.5)
    \put(0.5,0.8){\framebox(2.2,0.8){AVAILABLE}}
    \put(4.5,0.8){\framebox(2.2,0.8){PENDING}}
    \put(8.5,0.8){\framebox(2.2,0.8){APPROVED}}
    \put(12,0.8){\framebox(1.8,0.8){RETURNED}}
    
    \put(2.7,1.2){\vector(1,0){1.8}}
    \put(2.8,1.4){{\tiny Request}}
    
    \put(6.7,1.2){\vector(1,0){1.8}}
    \put(6.8,1.4){{\tiny Approve}}
    
    \put(10.7,1.2){\vector(1,0){1.3}}
    \put(10.9,1.4){{\tiny Return}}
    
    % Arc back
    \put(12.9,1.6){\line(0,1){0.8}}
    \put(12.9,2.4){\line(-1,0){11.3}}
    \put(1.6,2.4){\vector(0,-1){0.8}}
    \put(5.0,2.5){{\tiny Make Available Again}}
\end{picture}
...
\begin{enumerate}
    ...
    \item \textbf{Application Approval:} When an owner approves a specific application (\texttt{status = "APPROVED"}), the service triggers a database transaction that automatically marks all other applications for that announcement as \texttt{DENIED}.
...
By utilizing this token-based architecture, components do not define hardcoded hex colors. Instead, they reference semantic tokens (e.g., \texttt{color: var(--color-text-primary)}). When the theme attribute changes, the browser updates colors across the DOM tree automatically, providing a smooth user experience.
...
\begin{itemize}
    \item \textbf{General Security:} \texttt{router.get('/feed', requireAuthentication, requireVerified, feedController.getFeed)} protects standard community data.
    \item \textbf{Administrative Restrictions:} \texttt{router.put('/admin/approve/:id', requireAuthentication, requireAdminRole, adminController.approveUser)} restricts administrative actions to authorized accounts.
\end{itemize}
